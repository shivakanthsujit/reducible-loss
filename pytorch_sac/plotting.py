...
DEFAULT_BASELINES = ["d4pg", "dqn_sticky", "rainbow_sticky", "human$", "impala"]
...
def plot_combined(name, ax, runs, methods, args, agg, lo=None, hi=None, clip=False):
    ...
    borders = sorted(
        [r.xs for r in runs if r.xs is not None], key=lambda x: np.nanmax(x)
    )[-1]
...
def parse_args():
    ...
    parser.add_argument("--models", nargs="+", default=[r".*"])
    ...
    args.models = [re.compile(p) for p in args.models]
